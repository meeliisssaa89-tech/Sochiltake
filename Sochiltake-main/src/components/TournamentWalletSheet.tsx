...
import { Copy, Check, ArrowDownCircle, ArrowUpCircle, X, Clock, Send, RefreshCw, Loader2 } from "lucide-react";
...
export function TournamentWalletSheet({
  ...
  /* History queries */
  const { data: historyDeposits = [], refetch: refetchDeposits } = useQuery({
  ...
  const { data: historyWithdrawals = [], refetch: refetchWithdrawals } = useQuery({
    queryKey: ["my_tournament_withdrawals", userId],
    queryFn: async () => {
      if (!userId) return [];
      const { data } = await supabase
  ...
  const submitWithdrawal = async () => {
    ...
    if (!withdrawAddr.trim()) { setWithdrawErr("Enter your wallet address"); return; }
    if (isNaN(amt) || amt < minWithdraw) { setWithdrawErr(`Minimum withdrawal is $${minWithdraw}`); return; }
    if (amt > balance) { setWithdrawErr("Insufficient balance"); return; }

    setWithdrawSubmitting(true); setWithdrawErr(null);
    try {
      const network = withdrawNetwork || activeMethod?.network || cryptoMethods[0]?.network || "TRC20";
      ...
      const { error: wErr } = await supabase.from("tournament_withdrawals").insert({
        user_id:    userId,
        network,
      ...
      if (wErr) {
        ...
        await supabase.from("tournament_balances")
          .upsert({ user_id: userId, amount: balance, updated_at: new Date().toISOString() }, { onConflict: "user_id" });
  ...
  /* Merge history: deposits + withdrawals, sorted by date */
  const mergedHistory = [
    ...historyDeposits.map((d: any) => ({ ...d, _kind: "deposit" as const })),
    ...historyWithdrawals.map((w: any) => ({ ...w, _kind: "withdrawal" as const })),
          ...
          >
            ...
            >
              ...
              >
                {([
                  ...
                  <button
                    ...
                    onClick={() => {
                      setTab(key);
                      if (key === "history") { refetchDeposits(); refetchWithdrawals(); }
                      setSubmitted(false); setSubmitErr(null);
                      setWithdrawSubmitted(false); setWithdrawErr(null);
                    }}
                          ...
                          {paymentMethods.map((m) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setSubmitErr(null); setDepositStatus(null);
                              }}
                        ...
                        <div className="space-y-3">
                          ...
                          >
                            ...
                            <input
                              ...
                              className="w-full bg-transparent text-sm text-white font-mono break-all outline-none select-all cursor-text"
                          ...
                          >
                            <p className="text-[11px] text-white/50 text-center">
                              Minimum deposit:{" "}
                              <strong className="text-white">${minDeposit} USDT</strong>
                              {" · "}Send only <strong className="text-white">{activeMethod.coin}</strong> on{" "}
                              <strong className="text-white">{activeMethod.network}</strong> network
                            </p>
                          </div>
                        </div>
                      )}
                          ...
                          {submitted ? (
                            <div className="space-y-3">
                              <div
                                ...
                                style={{
                                  background: `rgba(${statusColor(depositStatus || "pending")},0.08)`,
                                  border: `1px solid rgba(${statusColor(depositStatus || "pending")},0.25)`,
                                }}
                              >
                                <div
                                ...
                                <p className="text-xs text-white/40">
                                  {depositStatus === "approved"
                                    ...
                                    : "Your request is under review. It will be credited after admin approval."}
                                </p>
                              </div>
                              ...
                              {countdown > 0 && depositStatus === "pending" && (
                                ...
                                >
                                  ...
                                  <span className="text-sm font-mono text-white/60">
                                    Expected in <strong className="text-white">{formatCountdown(countdown)}</strong>
                                  </span>
                                </div>
                              )}
                  ...
                  {!withdrawEnabled ? (
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        ...
                        <p className="text-sm font-bold text-white">Withdrawal Requested</p>
                        <p className="text-xs text-white/40">
                          Your withdrawal request has been submitted. The admin will process it shortly.
                        </p>
                      </div>
                      <button
                        onClick={() => { setWithdrawSubmitted(false); setWithdrawAddr(""); setWithdrawAmt(""); }}
                        className="w-full text-xs text-white/40 underline text-center py-1"
                      >
                    ...
                    <>
                        ...
                        <div>
                          ...
                          <div className="flex gap-1.5 overflow-x-auto pb-0.5">
                            {cryptoMethods.map((m) => (
                              <button
                                key={m.id}
                                onClick={() => { setSelectedMethodId(m.id); setWithdrawNetwork(m.network); }}
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold shrink-0 transition-all"
                      ...
                      <div className="space-y-1.5">
                        <label className="text-xs font-semibold text-white/50 uppercase tracking-wider">
                          Your Wallet Address {activeMethod ? `(${activeMethod.network})` : cryptoMethods[0] ? `(${cryptoMethods[0].network})` : ""}
                        </label>
                        <input
                          value={withdrawAddr}
                          onChange={(e) => setWithdrawAddr(e.target.value)}
                          placeholder={`Enter your ${activeMethod?.coin || cryptoMethods[0]?.coin || "USDT"} address…`}
                          className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/25 outline-none transition-all"
                          style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }}
                        />
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        />
                        {withdrawAmt && (
                          <p className={`text-xs px-1 ${Number(withdrawAmt) > balance ? "text-red-400" : "text-white/40"}`}>
                            Available: ${balance.toFixed(2)}
                            {Number(withdrawAmt) > balance ? " — Insufficient balance" : ""}
                          </p>
                        )}
                      </div>
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          !withdrawAddr.trim() ||
                          !withdrawAmt ||
                          Number(withdrawAmt) < minWithdraw ||
                          Number(withdrawAmt) > balance ||
                          !userId
                        }
                        className="w-full py-3.5 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-40"
                      ...
                      <p className="text-[10px] text-white/30 text-center">
                        Available: ${balance.toFixed(2)} · Min: ${minWithdraw} · Amount reserved on submit
                      </p>
                    </>
                  )}
                </div>
              ...
              {tab === "history" && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-white/50 uppercase tracking-wider">Transaction History</p>
                    <button
                  ...
                  {!userId ? (
                    ...
                    <div className="py-8 text-center text-white/30 text-sm">No transactions yet</div>
                  ) : (
                    mergedHistory.map((item: any) => {
                      const isDeposit = item._kind === "deposit";
                      const accentColor = isDeposit ? "34,197,94" : "139,92,246";
                      return (
                        <div
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              >
                                {isDeposit
                                  ? <ArrowDownCircle className="w-4 h-4" style={{ color: `rgb(${accentColor})` }} />
                                  : <ArrowUpCircle className="w-4 h-4" style={{ color: `rgb(${accentColor})` }} />}
                              </div>
                              <div className="min-w-0">
                                <p className="text-sm font-bold text-white">
                                  {isDeposit
                                    ? `${item.amount_local} ${item.currency}${item.amount_usd ? ` ≈ $${Number(item.amount_usd).toFixed(2)}` : ""}`
                                    : `$${Number(item.amount_usd).toFixed(2)} USDT`}
                                </p>
                                <p className="text-[11px] text-white/40">
                                  {isDeposit ? `via ${item.method_name}` : `to ${item.address?.slice(0, 12)}…`}
                                </p>
                              </div>
                            </div>
                            <span
                              className="text-[10px] px-2 py-1 rounded-full font-bold shrink-0"
                              style={{ background: `rgba(${statusColor(item.status)},0.12)`, color: `rgb(${statusColor(item.status)})` }}
                            >
                              {statusLabel(item.status)}
                            </span>
                          </div>
                          {item.admin_note && (
                            <p className="text-[10px] text-white/40 italic">Admin: {item.admin_note}</p>
                          )}
                          <p className="text-[10px] text-white/25">{new Date(item.created_at).toLocaleString()}</p>
                        </div>
                      );
                    })
                  )}
