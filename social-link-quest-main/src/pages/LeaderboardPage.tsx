...
import { useLeaderboard, usePlatformStats, useReferrals, useUserTasks } from "@/hooks/useSupabaseData";
...
export function LeaderboardPage() {
  ...
  const myRank =
    leaderboard?.findIndex((e) => e.userId === user?.telegram_id) ?? -1;
  const myScore = myRank >= 0 ? leaderboard![myRank].score : 0;

  const botUsername = "CyberPulseBot"; // Replace with actual bot username
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      <motion.div
        ...
        className="glass-card rounded-2xl p-4 border-primary/20"
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              {t("inviteDesc")} • {referralData?.count || 0} referrals
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button
            ...
            size="sm"
            onClick={copyLink}
            className="flex-1 rounded-xl h-8 text-xs border-primary/30 text-primary hover:bg-primary/10"
          >
          ...
          <Button
            ...
            className="flex-1 rounded-xl h-8 text-xs bg-primary hover:bg-primary/90 text-primary-foreground"
          >
      ...
      <div className="flex bg-secondary rounded-xl p-1 gap-1">
        <button
          onClick={() => setRankType("tasks")}
          className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${
            rankType === "tasks" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"
          }`}
        >
          {t("tasksRanking")}
        </button>
        <button
          onClick={() => setRankType("referrals")}
          className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${
            rankType === "referrals" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"
          }`}
        >
          {t("referralRanking")}
        </button>
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="glass-card rounded-xl p-3 flex items-center gap-3 border-accent/20"
      >
        ...
        <div className="text-end">
          <p className="text-xs text-muted-foreground">{rankType === "tasks" ? t("tasksCompleted") : "Referrals"}</p>
