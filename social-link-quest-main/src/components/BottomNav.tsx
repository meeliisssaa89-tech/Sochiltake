...
import { Home, ListChecks, Trophy, Activity, User } from "lucide-react";
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 pointer-events-none safe-bottom">
      <div className="max-w-lg mx-auto px-3 pb-3">
        <nav className="pointer-events-auto bottom-nav-glass rounded-2xl">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className="relative flex flex-col items-center gap-0.5 py-1.5 px-3 min-w-[56px] transition-all"
                >
                  {isActive && (
                    <motion.div
                  ...
                  {tab.id === "activity" && (
                    <span className="absolute -top-0.5 right-2 z-20">
                      <UpdateIndicator />
                    </span>
                  )}
                </button>
