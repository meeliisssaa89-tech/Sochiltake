  ...
  try {
    ...
    switch (action) {
      ...
      case 'shortlink_list_payouts': {
        const status = String(payload?.status || '');
        // Fetch payouts from DB2, currencies from DB1 (cross-DB)
        let q = slAdmin
        ...
        if (error) return json({ error: error.message }, 400);
        // Enrich with currency info from DB1
        const currencyIds = [...new Set((payouts || []).map((p: any) => p.currency_id).filter(Boolean))];
        let currencyMap: Record<string, any> = {};
        if (currencyIds.length > 0) {
          const { data: currs } = await admin.from('currencies').select('id, symbol, name, icon_url').in('id', currencyIds);
          for (const c of currs || []) currencyMap[c.id] = c;
        }
      ...
      case 'shortlink_approve_payout': {
        ...
        // Load settings from DB2 to find the payout currency
        ...
        // Credit the user's main app balance (DB1)
        const { data: bal } = await admin
          .from('balances')
          .select('id, amount')
        ...
        // Bump publisher's lifetime_earnings in DB2
        ...
        // Mark payout as approved in DB2
      ...
      case 'shortlink_reject_payout': {
        ...
        // Refund the publisher's pending_balance in DB2
